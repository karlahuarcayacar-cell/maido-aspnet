...
- **UI/UX (Admin Pedidos):** Se modernizó el Monitor de Pedidos (`Admin/Pedidos.cshtml`). Se eliminaron los filtros de fechas y el `<select>` tradicional, sustituyéndolos por una barra interactiva de Chips para filtrar por estado. La tabla de órdenes ahora incluye tipografía dorada para los IDs, una paleta de insignias de estado (*badges*) con colores neón (ej. verde, morado, azul) y botones fantasma para visualizar detalles.
- **UI/UX (Admin Usuarios):** Se rediseñó por completo el listado de usuarios (`Admin/Usuarios.cshtml`). Se migró de una tabla básica a la "Data Grid Neón" envuelta en `glass-card`. Se implementó la generación dinámica de avatares circulares a partir de las iniciales del nombre de cada usuario. Se actualizaron los botones de activar/desactivar a formato `ghost button` (brillando en rojo o verde al pasar el ratón) y se personalizaron los colores de las alertas emergentes (SweetAlert2) para que coincidan con la paleta oscura premium del proyecto.
