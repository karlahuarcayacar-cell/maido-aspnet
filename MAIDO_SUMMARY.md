...
- **UI/UX (Admin Platillos):** Se rediseñó la tabla de gestión de platillos (`Admin/Platillos.cshtml`) para alinearla al diseño "Premium Dark". Se implementaron filtros estilo Dashboard Widget dentro de un `glass-card`, avatares de platillos circulares, botones fantasma (`ghost buttons`) con efectos hover neón para editar/eliminar, e indicadores de estado circulares de alta tecnología (leds verde/rojo).
